...
import io.vakt.config.KafkaProperties;
...
class NominationKafkaResourceIT {
    ...
    void setup() {
        ...
        NominationKafkaResource kafkaResource = new NominationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/nomination-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/nomination-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
